...
*/
  ...
  if(Master->current != NULL){
    ...
    if(Master->current->state == TERMINATED){
      ...
      printf("thread ready to be executed");
...
int my_pthread_create(my_pthread_t* thread, pthread_attr_t* attr, void *(*function)(void*), void* arg){
  ...
  // If the MTH has not been initialized, initialize it
   if(start == 0)
  {
	  start = 1;
...
int main(int argc, char* argv[]){
	...
	ctx_main.uc_stack.ss_sp = func_stack;
	if(!ctx_main.uc_stack.ss_sp){
	printf("Malloc didn't work :(\n");
	exit(EXIT_FAILURE);
	}
	ctx_main.uc_stack.ss_size = sizeof(func_stack); 
	// make execution context
	makecontext(&ctx_main, (void(*))main_test, 0);
